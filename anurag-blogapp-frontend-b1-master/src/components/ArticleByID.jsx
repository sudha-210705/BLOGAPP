...
function ArticleByID() {
  ...
  useEffect(() => {
    ...
    const getArticle = async () => {
      ...
      try {
        const res = await axios.get(`https://blogapp-ps2r.onrender.com/user-api/article/${id}`, { withCredentials: true });
  ...
  const toggleArticleStatus = async () => {
    ...
    try {
      const res = await axios.patch(
        `https://blogapp-ps2r.onrender.com/author-api/articles/${id}/status`,
  ...
  const addComment = async (commentObj) => {
    ...
    let res = await axios.put("https://blogapp-ps2r.onrender.com/user-api/articles", commentObj, { withCredentials: true });
    if (res.status === 200) {
      toast.success(res.data.message);
      setArticle(res.data.payload);
    }
